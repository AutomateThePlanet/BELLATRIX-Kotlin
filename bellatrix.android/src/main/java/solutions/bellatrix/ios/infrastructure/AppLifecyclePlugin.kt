...
class AppLifecyclePlugin : Plugin() {
    ...
    private fun getExecutionAppMethodLevel(memberInfo: Method): AppConfiguration? {
        ...
        if (executionAppAnnotation == null) {
            ...
            return AppConfiguration(true)
        }
        return AppConfiguration(executionAppAnnotation.lifecycle, executionAppAnnotation.androidVersion, executionAppAnnotation.deviceName, executionAppAnnotation.appPath, executionAppAnnotation.appPackage, executionAppAnnotation.appActivity)
    }
    ...
    private fun getExecutionAppClassLevel(type: Class<*>): AppConfiguration {
        ...
        if (executionAppAnnotation == null) {
            ...
            val defaultAndroidVersion: String = ConfigurationService.get<AndroidSettings>().defaultAndroidVersion
            val defaultDeviceName: String = ConfigurationService.get<AndroidSettings>().defaultDeviceName
            return AppConfiguration(defaultLifecycle, defaultAndroidVersion, defaultDeviceName, defaultAppPath, defaultAppPackage, defaultAppActivity)
        } else if (executionAppAnnotation.isMobileWebTest) {
            return AppConfiguration(true)
        }
        return AppConfiguration(executionAppAnnotation.lifecycle, executionAppAnnotation.androidVersion, executionAppAnnotation.deviceName, executionAppAnnotation.appPath, executionAppAnnotation.appPackage, executionAppAnnotation.appActivity)
