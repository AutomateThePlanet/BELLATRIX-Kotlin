...
class IdContainingFindStrategy(value: String) : FindStrategy(value) {
    private val ID_CONTAINING_EXPRESSION = "new UiScrollable(new UiSelector()).scrollIntoView(new UiSelector().resourceIdMatches(\\\".*%s.*\\\"));"
    override fun findElement(driver: AndroidDriver<MobileElement>): MobileElement {
        return driver.findElementByAndroidUIAutomator(String.format(ID_CONTAINING_EXPRESSION, value))
    }

    override fun findAllElements(driver: AndroidDriver<MobileElement>): List<MobileElement> {
        return driver.findElementsByAndroidUIAutomator(String.format(ID_CONTAINING_EXPRESSION, value))
    }
    ...
    override fun toString(): String {
        return String.format("id containing %s", value)
    }
}