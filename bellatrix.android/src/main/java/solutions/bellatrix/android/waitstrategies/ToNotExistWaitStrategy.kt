...
class ToNotExistWaitStrategy : WaitStrategy {
    constructor() {
        timeoutInterval = ConfigurationService.get<AndroidSettings>().timeoutSettings.elementToNotExistTimeout
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementNotExists(getWrappedAndroidDriver(), findStrategy) }
        waitUntil(func)
    }

    private fun <TFindStrategy : FindStrategy> elementNotExists(searchContext: AndroidDriver<MobileElement>, findStrategy: TFindStrategy): Boolean {
        return try {
