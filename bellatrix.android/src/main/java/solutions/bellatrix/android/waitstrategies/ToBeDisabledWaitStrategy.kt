...
class ToBeDisabledWaitStrategy : WaitStrategy {
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementIsDisabled(getWrappedAndroidDriver(), findStrategy) }
        waitUntil(func)
    }

    private fun <TFindStrategy : FindStrategy> elementIsDisabled(searchContext: AndroidDriver<MobileElement>, findStrategy: TFindStrategy): Boolean {
