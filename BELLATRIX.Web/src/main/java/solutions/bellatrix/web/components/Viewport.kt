...
class Viewport : LayoutComponent {
    ...
    init {
        ...
        val viewportWidth: Int =
            javaScriptService.execute("return Math.max(document.documentElement.clientWidth, window.innerWidth || 0);")
                .toString().toInt()
        val viewportHeight: Int =
            javaScriptService.execute("return Math.max(document.documentElement.clientHeight, window.innerHeight || 0);")
                .toString().toInt()
