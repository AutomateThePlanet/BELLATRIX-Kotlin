...
class ToNotExistWaitStrategy : WaitStrategy {
    constructor() {
        timeoutInterval = ConfigurationService.get<DesktopSettings>().timeoutSettings.elementToNotExistTimeout
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementNotExists(getWrappedDriver(), findStrategy) }
        waitUntil(func)
    }

    private fun <TFindStrategy : FindStrategy> elementNotExists(searchContext: WindowsDriver<WebElement>, findStrategy: TFindStrategy): Boolean {
        return try {
