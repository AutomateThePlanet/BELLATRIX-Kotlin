...
class ToHaveContentWaitStrategy : WaitStrategy {
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementHasContent(getWrappedDriver(), findStrategy) }
        waitUntil(func)
    }

    private fun <TFindStrategy : FindStrategy> elementHasContent(searchContext: WindowsDriver<WebElement>, findStrategy: TFindStrategy): Boolean {
        return try {
