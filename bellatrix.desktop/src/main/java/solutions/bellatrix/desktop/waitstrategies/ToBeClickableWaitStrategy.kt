...
class ToBeClickableWaitStrategy : WaitStrategy {
    constructor() {
        timeoutInterval = ConfigurationService.get<DesktopSettings>().timeoutSettings.elementToBeClickableTimeout
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementIsClickable(getWrappedDriver(), findStrategy) }
        waitUntil(func)
    }
