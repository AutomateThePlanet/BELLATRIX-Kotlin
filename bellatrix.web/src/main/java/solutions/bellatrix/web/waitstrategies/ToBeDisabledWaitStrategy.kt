...
class ToBeDisabledWaitStrategy : WaitStrategy {
    constructor() : super() {
        timeoutInterval = ConfigurationService.get<WebSettings>().timeoutSettings.elementNotToBeVisibleTimeout
    ...
    override fun waitUntil(searchContext: SearchContext, by: By) {
        waitUntil { x: SearchContext -> elementIsDisabled(searchContext, by) }
    }

    private fun elementIsDisabled(searchContext: SearchContext, by: By): Boolean {
