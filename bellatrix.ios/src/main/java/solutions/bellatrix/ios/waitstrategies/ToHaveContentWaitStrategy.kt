...
class ToHaveContentWaitStrategy : WaitStrategy {
    constructor() {
        timeoutInterval = ConfigurationService.get<IOSSettings>().timeoutSettings.elementToHaveContentTimeout
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementHasContent(getWrappedIOSDriver(), findStrategy) }
        waitUntil(func)
    }

    private fun <TFindStrategy : FindStrategy> elementHasContent(searchContext: IOSDriver<MobileElement>, findStrategy: TFindStrategy): Boolean {
        return try {
