...
class ToBeClickableWaitStrategy : WaitStrategy {
    constructor() {
        timeoutInterval = ConfigurationService.get<IOSSettings>().timeoutSettings.elementToBeClickableTimeout
    ...
    override fun <TFindStrategy : FindStrategy> waitUntil(findStrategy: TFindStrategy) {
        val func = Function { w: WebDriver -> elementIsClickable(getWrappedIOSDriver(), findStrategy) }
        waitUntil(func)
    }

    private fun <TFindStrategy : FindStrategy> elementIsClickable(searchContext: IOSDriver<MobileElement>, findStrategy: TFindStrategy): Boolean {
