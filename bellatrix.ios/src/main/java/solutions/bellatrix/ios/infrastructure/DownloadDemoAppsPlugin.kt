...
class DownloadDemoAppsPlugin : Plugin() {
    ...
    override fun preBeforeClass(type: Class<*>) {
        ...
        val shouldDownloadDemoApps: Boolean = ConfigurationService.get<IOSSettings>().downloadDemoApps
        if (shouldDownloadDemoApps) {
            downloadToUsersFolder("https://github.com/AutomateThePlanet/BELLATRIX/releases/download/1.2/TestApp.app.zip")
        }
    }
}