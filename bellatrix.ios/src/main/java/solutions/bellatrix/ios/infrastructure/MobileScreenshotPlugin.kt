...
class MobileScreenshotPlugin(isEnabled: Boolean?) : ScreenshotPlugin(isEnabled!!) {
    override fun takeScreenshot(screenshotSaveDir: String, filename: String) {
        val screenshot = (getWrappedIOSDriver() as TakesScreenshot?)!!.getScreenshotAs(OutputType.FILE)
    ...
    companion object {
        fun of(): MobileScreenshotPlugin {
            val isEnabled: Boolean = ConfigurationService.get<IOSSettings>().screenshotsOnFailEnabled
